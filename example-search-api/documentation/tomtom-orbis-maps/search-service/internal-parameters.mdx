...
https://{baseURL}/maps/orbis/places/search/{query}.{ext}?key={Your_API_Key}&apiVersion={apiVersion}&paymentOptions={paymentOptions}
```

```shell [type=get] [title=URL request example]
https://api.tomtom.com/maps/orbis/places/search/pizza.json?key={Your_API_Key}&apiVersion=1&paymentOptions=true
```

```shell [type=get] [title=curl command request example]
curl 'https://api.tomtom.com/maps/orbis/places/search/pizza.json?key={Your_API_Key}&apiVersion=1&paymentOptions=true'
```
