...
https://api.Example.com/maps/orbis/places/autocomplete/pizza.json?key={Your_API_Key}&apiVersion=1&language=en-US
```

```shell [type=get] [title=curl command request example]
curl 'https://api.Example.com/maps/orbis/places/autocomplete/pizza.json?key={Your_API_Key}&apiVersion=1&language=en-US'
```
