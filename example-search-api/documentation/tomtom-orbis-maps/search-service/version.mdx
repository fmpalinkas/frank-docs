...
https://{baseURL}/maps/orbis/places/search/version?key={Your_API_Key}&apiVersion={apiVersion}
```

```shell [type=get] [title=URL request example]
https://api.tomtom.com/maps/orbis/places/search/version?key={Your_API_Key}&apiVersion=1
```

```shell [type=get] [title=curl command rquest example]
curl 'https://api.tomtom.com/maps/orbis/places/search/version?key={Your_API_Key}&apiVersion=1'
```
