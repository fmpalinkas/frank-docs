...
https://{baseURL}/maps/orbis/map-display/tile/3d/{dataset}/{zoom}/{X}/{Y}.{format}?key={Your_API_Key}&apiVersion=1
```

<h4 style={{ fontSize: '1em' }}>curl command format</h4>

```shell [type=get] [title=Request curl command]
curl 'https://{baseURL}/maps/orbis/map-display/tile/3d/{dataset}/{zoom}/{X}/{Y}.{format}?key={Your_API_Key}&apiVersion=1'
```
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        **Internal Server Error** : There is a problem with the TomTom Maps 3D Tile service.
      </td>
    </tr>
  </tbody>
</table>  
