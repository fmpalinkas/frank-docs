...
https://api.tomtom.com/search/2/search/pizza.json?key={Your_API_Key}&paymentOptions=true
```

```shell [type=get] [title=curl command request example]
curl 'https://api.tomtom.com/search/2/search/pizza.json?key={Your_API_Key}&paymentOptions=true'
```
