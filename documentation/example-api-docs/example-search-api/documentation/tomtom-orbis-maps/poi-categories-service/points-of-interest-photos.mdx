...
https://{baseURL}/maps/New/places/poiPhoto?key={Your_API_Key}&apiVersion={apiVersion}&id={id}&height={height}&width={width}
```

```shell [type=get] [title=Request curl command format]
curl 'https://{baseURL}/maps/New/places/poiPhoto?key={Your_API_Key}&apiVersion={apiVersion}&id={id}&height={height}&width={width}'
```
