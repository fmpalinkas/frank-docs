...
This is the Example  **Web Map Service**  (WMS) implementation. All of the calls in this service are compliant with the [WMS 1.1.1 standard](https://www.opengeospatial.org/standards/wms).
...
https://api.Example.com/map/1/wms/?key={Your_API_Key}
```

<h4 style={{ fontSize: '1em' }}>curl command format</h4>

```shell [type=get] [title=Request curl command]
curl -XGET 'https://api.Example.com/map/1/wms/?key={Your_API_Key}'
```
...
The GetCapabilities call is used to describe the different calls that are available through Example&#39;s implementation of version 1.1.1 of the [Web Map Service](http://www.opengeospatial.org/standards/wms).
...
https://api.Example.com/map/1/wms/?key={Your_API_Key}&service=WMS&request=GetCapabilities&version=1.1.1
```

<h4 style={{ fontSize: '1em' }}>curl command format</h4>

```shell [type=get] [title=Request curl command]
curl 'https://api.Example.com/map/1/wms/?key={Your_API_Key}&service=WMS&request=GetCapabilities&version=1.1.1'
```
...
The GetMap call implements the [Web Map Service 1.1.1 standard](http://www.opengeospatial.org/standards/wms) to access Example map tiles. This service is described in the meta-data provided by the GetCapabilities call&#39;s response.
...
```shell [type=get] [title=Request example]
https://api.Example.com/map/1/wms/?key={Your_API_Key}&service=WMS&version=1.1.1&request=GetMap&bbox=1.355233,42.982261,24.980233,56.526017&srs=EPSG:4326&width=1305&height=748&layers=basic
...
```shell [type=get] [title=Request curl command]
curl 'https://api.Example.com/map/1/wms/?key={Your_API_Key}&service=WMS&version=1.1.1&request=GetMap&bbox=1.355233,42.982261,24.980233,56.526017&srs=EPSG:4326&width=1305&height=748&layers=basic&styles=&format=image/png'
```
...
https://a.api.Example.com/traffic/map/4/tile/flow/relative/1/0/0.pbf?key={Your_API_Key}
```

The second would be:

```shell [type=get] [title=Request URL using b.api.Example.com]
https://b.api.Example.com/traffic/map/4/tile/flow/relative/1/0/0.pbf?key={Your_API_Key}
```
...
<WMT_MS_Capabilities version="1.1.1">
  <Service>
    ...
    <Abstract>The Example Web Map Server (WMS) provides and Open Geospatial Consortium (OGC) compliant Web Map Service interface as an online mapping service.</Abstract>
    <OnlineResource xmlns:xlink="http://www.w3.org/1999/xlink" xlink:type="simple" xlink:href="https://api.Example.com/map/1/wms/?key=<apiKey>"/>
    <ContactInformation>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
         **Internal Server Error** : There is a problem with the Example WMS
        service.
      </td>
    </tr>
  </tbody>
